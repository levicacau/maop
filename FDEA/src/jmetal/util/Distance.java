...
// 	   front.get(n).setCrowdingDistance(front.get(n).getCrowdingDistance()/10);    Ð§¹û²»ºÃ£¿ËùÓÐµÄÓµ¼·¾àÀëµÄÖµ½ÏÐ¡ ÎÞ·¨¹À¼ÆÖÖÈºµÄ¶àÑùÐÔ¡£
...
public void crowdingDistanceAssignment5(SolutionSet solutionSet, int nObjs) {
    ...
    for (int i = 0; i<nObjs; i++) {          
      // Sort the population by Obj n            
      front.sort(new ObjectiveComparator(i));
      objectiveMin[i] = front.get(0).getObjective(i);   //×îÐ¡µÄÖµ
//      objetiveMinn = front.get(0).getObjective(i);      
//      objetiveMaxn = front.get(front.size()-1).getObjective(i); 
      objectiveMax[i] = front.get(front.size()-1).getObjective(i); //×î´óµÄÖµ
      
      double maxDistance = -1.0e+30;
	   double minDistance = 1.0e+30;  
      
      //Set de crowding distance   ÕâÀï½«±ß½çµãµÄÓµ¼·¾àÀëÉèÖÃÎªÎÞÇî´óÊÇ²»ºÏÀíµÄ£¬ÒòÎªÒª¿¼ÂÇµ½±È±ß½çµã¿ÉÄÜÌØ±ðµÄ²î                                                 
      for (int j = 1; j < size-1; j++) {
          distance[j] =front.get(j+1).getObjective(i) - front.get(j-1).getObjective(i) ;                    
          distance[j] = distance[j] /(objectiveMax[i] -  objectiveMin[i]) ;  			   
          distance[j] += front.get(j).getCrowdingDistance();                
...
//   }
   ...
   //------------------------¶Ô±ß½çµÄµã²»ÔÙÉèÖÃÎªÎÞÇî´ó£¬¶øÊÇÉèÖÃÎªÆ½¾ùµÄÓµ¼·¾àÀëstart----------------//
...
//    }
 ...
 //------------------------¶Ô±ß½çµÄµã²»ÔÙÉèÖÃÎªÎÞÇî´ó£¬¶øÊÇÉèÖÃÎªÆ½¾ùµÄÓµ¼·¾àÀëend----------------//
...
public void crowdingDistanceAssignment44(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚÒ»Ì×Êý¾Ý
    int size = solutionSet.size();        
...
public void crowdingDistanceAssignment4(SolutionSet solutionSet, int nObjs) {//SDE+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
...
public void crowdingDistanceAssignment45(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
...
public void crowdingDistanceAssignment46(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
...
public void crowdingDistanceAssignment54(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
...
public void crowdingDistanceAssignment55(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
  ...
  /*for (int i = 0; i<nObjs; i++) { 
	  ...
	  objectiveMin[i] = front.get(0).getNormalizedObjective(i);   //×îÐ¡µÄÖµ 
      objectiveMax[i] = front.get(front.size()-1).getNormalizedObjective(i); //×î´óµÄÖµ
      for (int j = 1; j < size-1; j++) {
...
public void crowdingDistanceAssignment77(SolutionSet solutionSet, int nObjs) {//Óµ¼·¶È¾àÀë+µÚ¶þÌ×Êý¾Ý
    int size = solutionSet.size();        
