	...
	public static double calcular(ArrayList<double[]> front, String tempname) throws IOException{
		...
		try{
			...
			if(front.get(0).length <= 10){
				comando+="hv/wfg /tmp/temp"+tempname+".txt"+ref+" | head -1";
...
//		if(dados.size() > 0)
	...
	public static double calculaHipervolumeAmostragem(ArrayList<double[]> front) throws IOException{
	
    ArrayList<double[]> dados = new ArrayList<double[]>();
    for(int i=0;i<front.size();i++){
        double[] d = new double[front.get(i).length];
        for (int o = 0; o < front.get(i).length; o++) {
            d[o] = normalize(front.get(i)[o], menoresValores[o], maioresValores[o]);
        }
        dados.add(d);
    }
	
// 	for(int i=0;i<dados.size();i++)
//     for(int o=0;o<dados.get(i).length;o++){ //objetivos da solucao
// //         System.out.print(normalize(dados.get(i)[o], menoresValores[o], maioresValores[o])+" ");
//         System.out.print(dados.get(i)[o]+" ");
//     }
//     System.out.println();
