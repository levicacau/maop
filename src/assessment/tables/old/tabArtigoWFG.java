...
// 		String metrics[]={"Hypervolume"};
		...
		table = new String[2+(titles.length*objectives.length)][2+(metrics.length*WFG)];
		...
		for(int j=2;j<table[0].length;j++){
			if((j-2)%metrics.length==0)
				if((((j-2)/metrics.length)+1) != WFG)
					table[0][j]="\\multicolumn{"+metrics.length+"}{c|}{WFG"+(((j-2)/metrics.length)+1)+"}";
				else
					table[0][j]="\\multicolumn{"+metrics.length+"}{c}{WFG"+(((j-2)/metrics.length)+1)+"}";
	...
	public static void readData(int algorithms, int objectiveNumber, int WFG, String metric)throws IOException{ //wfg, algoritmo, linha
		...
		if(metric.toUpperCase().equals("$GD_P$"))
			scanner = new Scanner( new FileInputStream( caminho+"/all-wfg"+WFG+"-gd.txt" ) );
		if(metric.toUpperCase().equals("$IGD_P$"))
			scanner = new Scanner( new FileInputStream( caminho+"/all-wfg"+WFG+"-igd.txt" ) );
		if(metric.toUpperCase().equals("$R_2$"))
			scanner = new Scanner( new FileInputStream( caminho+"/all-wfg"+WFG+"-r2.txt" ) );
		if(metric.toUpperCase().equals("HYPERVOLUME")){
			scanner = new Scanner( new FileInputStream( caminho+"/all-wfg"+WFG+"-hv.txt" ) );
			min=false;
		}
